...
function applyDrivingPrice() {
  if (
    form.value.businessType !== 'DRIVING_SCHOOL' ||
...
function validate() {
  if (!form.value.name?.trim() || !/^1\d{10}$/.test(form.value.phone || ''))
    return '请填写姓名和正确手机号';
  if (form.value.businessType !== 'DRIVING_SCHOOL' && !form.value.businessNumber?.trim())
    return '请填写业务号码';
  if (form.value.businessType === 'CAMPUS_NETWORK' && !form.value.studentNo?.trim())
    return '请填写学号';
  if (
    form.value.businessType === 'CAMPUS_NETWORK' &&
    !/^\d{6}$/.test(form.value.idCardLastSix || '')
  )
    return '身份证后六位应为 6 位数字';
  if (form.value.businessType === 'DRIVING_SCHOOL' && Number(form.value.paymentAmount) <= 0)
    return '驾校价格尚未配置';
  if (form.value.businessType === 'RENEWAL' && Number(form.value.renewalAmount) <= 0)
    return '续费金额必须大于 0';
  return '';
}
