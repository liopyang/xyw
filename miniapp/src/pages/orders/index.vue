...
<template>
  <view class="page list-page">
    <view v-if="user.role !== 'AGENT'" class="card empty-card">
    ...
    <view v-if="user.role === 'AGENT' && !loading && rows.length === 0" class="empty">暂无订单</view>
    <view v-if="user.role === 'AGENT' && rows.length" class="load-state">
      {{ loading ? '正在加载...' : finished ? `已加载全部 ${total} 条订单` : '上拉加载更多' }}
    </view>
    <button v-if="user.role === 'AGENT'" class="primary add" @click.stop="add">提交订单</button>
  </view>
</template>

<style scoped>
.list-page { padding-bottom: 180rpx; }
.created-at { margin: 14rpx 0 0; }
.empty-card { color: #697586; line-height: 1.7; }
