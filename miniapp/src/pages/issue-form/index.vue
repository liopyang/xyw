...
async function save() {
  ...
  try {
    ...
    try {
      await Promise.all(
        images.value.map((path) => uploadFile(`/mini/issues/${result.id}/images`, path)),
      );
      uni.showToast({ title: '提交成功' });
    } catch {
      uni.showToast({
        title: '问题已提交，部分图片上传失败',
        icon: 'none',
        duration: 2500,
      });
    }
...
<template>
  <view class="page">
    <view class="card">
      ...
      <picker
        :range="issueTypeNames"
        @change="form.issueType = issueTypes[Number($event.detail.value)]"
      >
