...
onLoad(async (query: Record<string, string> = {}) => {
  ...
  if (businessTypes.includes(query.type as BusinessType))
    form.businessType = query.type as BusinessType;
...
function validate() {
  ...
  if (form.businessType !== 'DRIVING_SCHOOL' && !form.businessNumber.trim())
    return '请填写业务号码';
  if (form.businessType === 'CAMPUS_NETWORK' && !form.studentNo.trim()) return '请填写学号';
  if (form.businessType === 'CAMPUS_NETWORK' && !/^\d{6}$/.test(form.idCardLastSix))
    return '身份证后六位应为 6 位数字';
  if (form.businessType === 'DRIVING_SCHOOL' && form.paymentAmount <= 0)
    return '驾校价格尚未配置，请联系管理员';
...
<template>
  <view class="page">
    <view class="card">
      ...
      <picker
        :range="businessTypeNames"
        @change="form.businessType = businessTypes[Number($event.detail.value)]"
      >
      ...
      <picker
        :range="sourceChannelNames"
        @change="form.sourceChannel = sourceChannels[Number($event.detail.value)]"
      >
      ...
      <template v-if="form.businessType !== 'DRIVING_SCHOOL'">
        <text class="label">
          {{ form.businessType === 'CAMPUS_NETWORK' ? '新办号码' : '业务号码' }}
        </text>
        <input
      ...
      <template v-if="form.businessType === 'DRIVING_SCHOOL'">
        ...
        <picker
          :range="licenseTypes"
          @change="form.licenseType = licenseTypes[Number($event.detail.value)]"
        >
          <view class="input picker-value">{{ form.licenseType }}</view>
        </picker>
        <text class="label">班型</text>
        <picker
          :range="classTypeNames"
          @change="form.classType = classTypes[Number($event.detail.value)]"
        >
          <view class="input picker-value">
            {{ form.classType === 'NORMAL' ? '普通班' : '全包班' }}
          </view>
        </picker>
        <text class="label">缴费数额</text>
        <view class="input picker-value price">
          {{ form.paymentAmount > 0 ? `¥${form.paymentAmount}` : '价格未配置' }}
        </view>
      ...
      <template v-if="form.businessType === 'RENEWAL'">
        ...
        <input
          v-model.number="form.renewalAmount"
          class="input"
          type="digit"
        />
      </template>
