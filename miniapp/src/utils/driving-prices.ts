...
export function normalizeDrivingPrices(
  homeData: Record<string, unknown> | null | undefined,
): DrivingPriceMap {
  ...
  return {
    C1_NORMAL: numberValue(
      raw.C1_NORMAL ?? raw.drivingC1NormalPrice ?? c1.NORMAL ?? homeData?.drivingC1NormalPrice,
    ),
    C1_FULL: numberValue(
      raw.C1_FULL ?? raw.drivingC1FullPrice ?? c1.FULL ?? homeData?.drivingC1FullPrice,
    ),
    C2_NORMAL: numberValue(
      raw.C2_NORMAL ?? raw.drivingC2NormalPrice ?? c2.NORMAL ?? homeData?.drivingC2NormalPrice,
    ),
    C2_FULL: numberValue(
      raw.C2_FULL ?? raw.drivingC2FullPrice ?? c2.FULL ?? homeData?.drivingC2FullPrice,
    ),
  };
}
...
export function drivingPrice(
  prices: DrivingPriceMap,
