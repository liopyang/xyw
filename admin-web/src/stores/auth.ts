...
export const useAuthStore = defineStore('auth', () => {
  ...
  async function login(username: string, password: string) {
    const res = await request.post<never, ApiResponse<{ token: string; user: UserInfo }>>(
      '/auth/login',
      { username, password },
    );
    token.value = res.data.token;
    user.value = res.data.user;
    localStorage.setItem('campus_token', token.value);
    localStorage.setItem('campus_user', JSON.stringify(user.value));
  }
  async function fetchMe() {
    const res = await request.get<never, ApiResponse<UserInfo>>('/auth/me');
    user.value = res.data;
    localStorage.setItem('campus_user', JSON.stringify(user.value));
  }
