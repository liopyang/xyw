    ...
    <div class="panel table-panel">
      ...
      >
        ...
        <el-table-column
          ...
          label="提交人"
        />
        <el-table-column label="提交人类型">
          <template #default="{ row }">
            {{ row.submitterType === 'AGENT' ? '代理' : '普通用户' }}
          </template>
        </el-table-column>
        <el-table-column
          label="联系电话"
        ...
        <el-table-column
          ...
          width="130"
        >
          <template #default="{ row }">{{ typeNames[row.issueType] }}</template>
        </el-table-column>
        <el-table-column
          prop="description"
          label="问题描述"
        ...
        >
          <template #default="{ row }">
            <el-button
              link
              type="primary"
              @click="router.push(`/issues/${row.id}`)"
            >
              处理
            </el-button>
          </template>
