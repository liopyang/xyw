...
const todos = reactive<Todos>({
  pendingOrders: 0,
  unexportedNetworks: 0,
  pendingIssues: 0,
  processingIssues: 0,
  monthlyAgentOrders: 0,
});
...
async function loadTrend() {
  const params: TrendQuery = {
    businessType: trendParams.businessType,
    range: trendParams.range,
  };
    ...
    series: [
      {
        ...
        areaStyle: {
          color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
            { offset: 0, color: 'rgba(45,115,235,.22)' },
            { offset: 1, color: 'rgba(45,115,235,0)' },
          ]),
        },
      },
    ],
...
async function load() {
  loading.value = true;
  try {
    const [cardResult, todoResult] = await Promise.all([
      dashboardApi.cards(),
      dashboardApi.todos(),
    ]);
...
watch(
  () => [rankParams.type, rankParams.range],
  () => loadRanking(),
);
  ...
  >
    <div class="heading">
      <div>
        ...
        <p class="page-desc">掌握今日业务进展与关键待办</p>
      </div>
      <span class="date">数据更新于刚刚</span>
    </div>
    <div class="cards">
      <div
        v-for="card in cards"
      ...
      >
        <div class="stat-top">
          <span
            class="stat-icon"
            :style="{
              background: meta[card.businessType].color + '16',
              color: meta[card.businessType].color,
            }"
          >
            <component :is="meta[card.businessType].icon" />
          </span>
          <b>{{ meta[card.businessType].name }}</b>
        </div>
        <div class="figures">
          <div>
            <strong>{{ card.today }}</strong>
            <span>今日订单</span>
          </div>
          <i></i>
          <div>
    ...
    <div class="grid">
      <section class="panel chart-panel">
        <div class="panel-head">
          ...
          <div class="controls">
            <el-select
              v-model="trendParams.businessType"
              style="width: 130px"
            >
              <el-option
            ...
            <el-select
              v-model="trendParams.range"
              style="width: 110px"
              @change="selectTrendRange"
            >
              <el-option
                label="近7天"
                value="7d"
      ...
      <section class="panel rank-panel">
        ...
        <div class="rank-list">
          <div
            v-for="(item, index) in ranking.slice(0, 5)"
            :key="item.agentId"
            class="rank-row"
          >
            <span :class="['rank-no', index < 3 ? 'top' : '']">{{ index + 1 }}</span>
            <span class="rank-avatar">{{ item.agentName.slice(0, 1) }}</span>
            <b>{{ item.agentName }}</b>
            <strong>
              {{ item.count }}
              <small>单</small>
    ...
    <section class="panel todo-panel">
      ...
      <div class="todos">
        <button @click="todo('/orders', { auditStatus: 'PENDING' })">
          <span class="dot blue"></span>
          <div>
            <strong>{{ todos.pendingOrders }}</strong>
            <p>待确认订单</p>
          </div>
          <ArrowRight />
        </button>
        ...
        >
          ...
          <div>
            <strong>{{ todos.unexportedNetworks }}</strong>
            <p>校园网未导出</p>
          </div>
          <ArrowRight />
        </button>
        <button @click="todo('/issues', { status: 'PENDING' })">
          <span class="dot red"></span>
          <div>
            <strong>{{ todos.pendingIssues }}</strong>
            <p>待处理问题</p>
          </div>
          <ArrowRight />
        </button>
        <button @click="todo('/issues', { status: 'PROCESSING' })">
          <span class="dot purple"></span>
          <div>
            <strong>{{ todos.processingIssues }}</strong>
            <p>处理中问题</p>
          </div>
          <ArrowRight />
        </button>
        <button @click="todo('/agents', { range: 'month' })">
          <span class="dot green"></span>
          <div>
            <strong>{{ todos.monthlyAgentOrders }}</strong>
            <p>本月代理订单</p>
          </div>
          <ArrowRight />
        </button>
