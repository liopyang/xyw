...
async function loadAgents() {
  ...
  agents.value = res.data.records.map((item: AgentOption) => ({
    id: item.id,
    name: item.name,
  }));
}
...
async function action(kind: 'confirm' | 'void' | 'restore', row: Order) {
  ...
  try {
    await ElMessageBox.confirm(
      `确定要${labels[kind]}订单 ${row.orderNo} 吗？`,
      `${labels[kind]}订单`,
      { type: kind === 'void' ? 'warning' : 'info' },
    );
  } catch {
    return;
  }
...
async function exportOne(row: Order) {
  ...
  try {
    const blob = await orderApi.exportNetwork({
      orderId: row.id,
...
<template>
  <div class="page">
    ...
    <div class="filter-bar panel">
      ...
      >
        <el-option
          v-for="(value, key) in sourceNames"
          :key="key"
          :label="value"
          :value="key"
        />
      ...
      <transition name="search-slide">
        ...
        >
          <el-input
            ...
            clearable
            placeholder="搜索订单编号、姓名、手机号、业务号码、代理或备注"
            @keyup.enter="search"
            @clear="search"
          >
            <template #append>
      ...
      >
        ...
        >
          <template #default="{ row }">
            <el-tag>{{ typeNames[row.businessType as keyof typeof typeNames] }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column
          prop="name"
        ...
        >
          <template #default="{ row }">
            {{ sourceNames[row.sourceChannel as keyof typeof sourceNames] }}
          </template>
        </el-table-column>
        <el-table-column
          prop="agentName"
        ...
        >
          <template #default="{ row }">
            ...
            >
              {{ row.auditStatus === 'CONFIRMED' ? '已确认' : '待确认' }}
            </el-button>
          </template>
        </el-table-column>
        <el-table-column
          ...
          <template #default="{ row }">
            <el-button
              ...
              :class="[
                'export-tag',
                {
                  'is-disabled': row.deleted,
                  'is-loading': exportingId === row.id,
                },
              ]"
              @click="exportOne(row)"
            >
              {{ row.exportStatus === 'NOT_EXPORTED' ? '未导出' : '已导出' }}
            </el-button>
          </template>
        </el-table-column>
        <el-table-column
        ...
        >
          <template #default="{ row }">
            ...
            <el-button
              ...
              link
              type="primary"
              @click="router.push(`/orders/${row.id}?edit=1`)"
            >
              编辑
            </el-button>
            <el-button
              v-if="!row.deleted && row.auditStatus === 'PENDING'"
              link
              type="success"
            ...
            <el-button
              ...
              link
              type="danger"
              @click="action('void', row)"
            >
              作废
            </el-button>
            <el-button
              v-if="row.deleted && auth.isOwner"
              link
              type="warning"
              @click="action('restore', row)"
            >
              恢复
            </el-button>
          </template>
...
.export-tag.is-disabled {
  cursor: not-allowed;
  opacity: 0.65;
}
.export-tag.is-loading {
  cursor: wait;
  opacity: 0.65;
}
.status-button {
