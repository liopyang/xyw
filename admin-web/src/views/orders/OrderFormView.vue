...
const needsBusinessNumber = computed(() =>
  ['CAMPUS_CARD', 'CAMPUS_NETWORK', 'RENEWAL'].includes(form.businessType),
);
...
const rules = computed<FormRules>(() => ({
  ...
  businessNumber: needsBusinessNumber.value
    ? [
        {
          required: true,
          message: `请输入${businessNumberLabel.value}`,
          trigger: 'blur',
        },
      ]
    : [],
  studentNo:
    form.businessType === 'CAMPUS_NETWORK'
  ...
  idCardLastSix:
    form.businessType === 'CAMPUS_NETWORK'
      ? [
          ...
          {
            pattern: /^\d{6}$/,
            message: '身份证后六位必须是 6 位数字',
            trigger: 'blur',
          },
        ]
      : [],
  licenseType:
    form.businessType === 'DRIVING_SCHOOL'
  ...
  paymentAmount:
    form.businessType === 'DRIVING_SCHOOL'
      ? [{ required: true, message: '请输入缴费数额', trigger: 'change' }]
      : [],
  renewalAmount:
...
async function load() {
  ...
  try {
    ...
    agents.value = agentResult.data.records.map((item: AgentOption) => ({
      id: item.id,
      name: item.name,
    }));
    ...
    <div class="form-panel panel">
      <div class="form-head">
        ...
        <el-button
          v-if="id && !editing"
          type="primary"
          @click="router.replace({ query: { edit: '1' } })"
        >
          编辑订单
        </el-button>
      </div>
      ...
      >
        <div class="form-grid">
          ...
          <el-form-item
            ...
            label="审核状态"
          >
            <el-tag :type="form.auditStatus === 'CONFIRMED' ? 'success' : 'warning'">
              {{ form.auditStatus === 'CONFIRMED' ? '已确认' : '待确认' }}
            </el-tag>
          ...
          <template v-if="form.businessType === 'DRIVING_SCHOOL'">
            ...
            >
              ...
              />
              <span class="field-hint">默认价格由业务配置带出，可手动修改</span>
            </el-form-item>
          </template>

          <el-form-item
            v-if="form.businessType === 'RENEWAL'"
            label="续费金额"
            prop="renewalAmount"
          >
            <el-input-number
              v-model="form.renewalAmount"
              :min="0"
              :precision="2"
              controls-position="right"
            />
