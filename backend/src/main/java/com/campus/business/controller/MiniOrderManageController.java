...
public class MiniOrderManageController {
  ...
  public ApiResponse<Void> update(
      @PathVariable Long id, @Valid @RequestBody MiniappController.MiniOrder request) {
    ...
    if ("DRIVING_SCHOOL".equals(businessType)
        && paymentAmount == null
    ...
    jdbc.update(
        "UPDATE biz_order SET"
            + " customer_name=:name,contact_phone=:phone,business_number=:businessNumber,remark=:remark,audit_status='PENDING'"
            + " WHERE id=:id",
        parameters);
  ...
  private Map<String, Object> one(Long id) {
    String sql =
        "SELECT o.id,o.order_no orderNo,o.business_type businessType,o.customer_name"
            ...
            + " sourceChannel,o.audit_status auditStatus,o.remark,o.created_at"
            + " createdAt,n.student_no studentNo,n.id_card_last_six idCardLastSix,n.export_status"
            + " exportStatus,d.license_type licenseType,d.class_type classType,d.payment_amount"
            + " paymentAmount,r.renewal_amount renewalAmount FROM biz_order o JOIN agent a ON"
            + " a.id=o.agent_id LEFT JOIN order_campus_network n ON n.order_id=o.id LEFT JOIN"
            + " order_driving_school d ON d.order_id=o.id LEFT JOIN order_renewal r ON"
            + " r.order_id=o.id WHERE o.id=:id AND o.deleted=0 AND a.user_id=:userId";
    List<Map<String, Object>> rows =
        jdbc.queryForList(sql, Map.of("id", id, "userId", SecurityUtils.current().id()));
    if (rows.isEmpty()) {
