...
public class AuthController {
    ...
    public record ChangePasswordRequest(
            ...
            @NotBlank(message = "请输入新密码") @Size(min = 8, max = 72, message = "新密码长度应为8至72位") String newPassword
    ) {
    }
    ...
    public ApiResponse<Void> changePassword(@Valid @RequestBody ChangePasswordRequest request) {
        ...
        if (user == null || user.getStatus() != 1 || !encoder.matches(request.currentPassword(), user.getPassword())) {
