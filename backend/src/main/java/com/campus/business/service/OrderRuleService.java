...
public class OrderRuleService {
  public static final Set<String> BUSINESS_TYPES =
      Set.of("CAMPUS_CARD", "CAMPUS_NETWORK", "DRIVING_SCHOOL", "RENEWAL");
  ...
  public void checkDuplicate(
      String businessType, String businessNumber, String phone, Long excludeId) {
    String field =
        businessNumber != null && !businessNumber.isBlank() ? "business_number" : "contact_phone";
    ...
    StringBuilder sql =
        new StringBuilder(
            "SELECT order_no,created_at FROM biz_order "
                + "WHERE deleted=0 AND business_type=:type AND "
                + field
                + "=:value AND created_at>=:cutoff");
    if (excludeId != null) {
